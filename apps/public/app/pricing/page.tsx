  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        />
        <Container className="pt-8 pb-12 text-center sm:pt-10">
            ...
            >
              {tier.featured && (
                <>
                  ...
                  <span className="absolute right-4 top-4 rounded-full bg-action px-2.5 py-1 text-xs font-bold text-white">
                    Popular
                  </span>
                </>
              )}
              <div className="flex flex-1 flex-col p-6">
                ...
                <ul className="mt-6 flex-1 space-y-3">
                  {tier.features.map((f) => {
                    const inherit = f.startsWith("Everything in");
                    return (
                      <li
                        key={f}
                        className={`flex items-start gap-2 text-sm ${
                          inherit ? "font-semibold text-foreground" : "text-muted"
                        }`}
                      >
                        <Check
                          size={16}
                          weight="bold"
                          className={`mt-0.5 shrink-0 ${inherit ? "text-steel" : "text-verdant"}`}
                          aria-hidden="true"
                        />
                        {f}
                      </li>
